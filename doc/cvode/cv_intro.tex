...
{\cvode} now supports a new ``matrix-embedded'' {\sunlinsol} type.  This type
...
module. See the optional input function \\\id{CVodeSetUseIntegratorFusedKernels}
for more information. As with other {\sundials} {\hip} features, this
capability is considered experimental and may change from version to version.
...
iterations may be retreived by calling \id{CVodeGetNumNonlinSolvIters}, the
cumulative number of failures with \id{CVodeGetNumNonlinSolvConvFails}, or both
with \id{CVodeGetNonlinSolvStats}.
...
performance on smaller problems when using {\cvode} with the {\nveccuda}
module. See the optional input function \id{CVodeSetUseIntegratorFusedKernels}
for more information. As with other {\sundials} {\cuda} features, this
capability is considered experimental and may change from version to version.
...
Added a new function, \id{CVodeSetMonitorFn}, that takes a user-function
...
Added a new function \id{CVodeGetLinSolveStats} to get the {\cvode} linear
solver statistics as a group.

Added the optional function \id{CVodeSetJacTimesRhsFn} to specify an alternative
right-hand side function for computing Jacobian-vector products with the
internal difference quotient approximation.

Added support for integrating IVPs with constraints using BDF methods and
projecting the solution onto the constraint manifold with a user defined
projection function. This implementation is accompanied by additions to user
documentation and {\cvode} examples. See \S\ref{ss:constraints} for more
information.
...
The function \id{CVodeSetLinearSolutionScaling} was added to enable or disable
