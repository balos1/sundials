...
%
\item Two new \id{N\_Vector} implementations, {\nvecmanyvector} and
...
multi-physics problems that couple distinct MPI-based simulations together
(see the \S\ref{ss:nvec_manyvector} and \S\ref{ss:nvec_mpimanyvector}for more
details). This implementation is accompanied by additions to user documentation
...
been added to the {\nvector} API. The new required operation, \id{N\_VGetLenth},
...
to provide user access to the underlying KLU solver structures.
%
...
parameters. For more information on the nonlinear system formulation and the
API functions see Chapter \ref{c:sunnonlinsol}.
%
\item Added a new {\sunnonlinsol} implementation, {\sunnonlinsolpetsc}, which
interfaces to the PETSc SNES nonlinear solver API.
%
