...
Added the function \id{CVodeSetNlsRhsFn} to supply an alternative right-hand
...
Added the function \id{CVodeSetLSNormFactor} to specify the factor for
converting between integrator tolerances (WRMS norm) and linear solver
tolerances (L2 norm) i.e., \id{tol\_L2 = nrmfac * tol\_WRMS}.

Added new functions \Id{CVodeComputeState}, and \Id{CVodeGetNonlinearSystemData}
...
The \id{CVodeF} function for forward integration with checkpointing is now
subject to a restriction on the number of time steps allowed to reach the output
time. This is the same restriction applied to the \id{CVode} function. The
default maximum number of steps is 500, but this may be changed using the
\id{CVodeSetMaxNumSteps} function. This change fixes a bug that could cause an
infinite loop in the \id{CVodeF} function.
...
iterations may be retreived by calling \id{CVodeGetNumNonlinSolvIters},
\id{CVodeGetSensNumNonlinSolvIters}, \id{CVodeGetStgrSensNumNonlinSolvIters},
the cumulative number of failures with \id{CVodeGetNumNonlinSolvConvFails},
\id{CVodeGetSensNumNonlinSolvConvFails},
\id{CVodeGetStgrSensNumNonlinSolvConvFails}, or both with
\id{CVodeGetNonlinSolvStats}, \id{CVodeGetSensNonlinSolvStats},
\id{CVodeGetStgrSensNonlinSolvStats}.
...
through a call to \id{CVodeSetMaxStepsBetweenJac} will need to increase the
provided value by 1 to achieve the same behavior as before. For greater clarity
the function \id{CVodeSetMaxStepsBetweenJac} has been deprecated and replaced
with \id{CVodeSetJacEvalFrequency}. Additionally, the function
\id{CVodeSetLSetupFrequency} has been added to set the frequency of calls to the
...
The \id{NVECTOR\_RAJA} module has been updated to mirror the \id{NVECTOR\_CUDA}
module. Notably, the update adds managed memory support to the
\id{NVECTOR\_RAJA} module. Users of the module will need to update any calls to
the \id{N\_VMake\_Raja} function because that signature was changed. This module
remains experimental and is subject to change from version to version.
...
Added the optional functions \id{CVodeSetJacTimesRhsFn} and
\id{CVodeSetJacTimesRhsFnB} to specify an alternative right-hand side function
...
The functions \id{CVodeSetLinearSolutionScaling} and
\id{CVodeSetLinearSolutionScalingB} were added to enable or disable the scaling
